...
import { User, SystemUserResponse } from '@/types/shared/auth';
...
function AdminPageContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (activeTab === 'users') {
        ...
        // Only fetch system users data when on system-users tab
  ...
  const handleCreateSystemUser = async (e: React.FormEvent) => {
    ...
    try {
      const _res = await apiPost<SystemUserResponse>('/api/system-users', systemUserForm);
  ...
  const handleUpdateSystemUser = async (e: React.FormEvent) => {
    ...
    try {
      const _res = await apiPut<SystemUserResponse>(
        `/api/system-users/${editingSystemUser.id}`,
        systemUserForm
      );
      setSystemUserForm({ name: '', email: '', avatar: '' });
      setShowSystemUserForm(false);
  ...
  const handleDeleteSystemUser = async (systemUser: SystemUserResponse) => {
    ...
    try {
      const _res = await apiDelete<SuccessResponse>(`/api/system-users/${systemUser.id}`);
                  ...
                  <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/20 pt-8 backdrop-blur-sm">
                    ...
                    >
                      ...
                      <form
                        id="system-user-form"
                        onSubmit={
                          editingSystemUser ? handleUpdateSystemUser : handleCreateSystemUser
                        }
                        className="space-y-4"
                      >
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setSystemUserForm({ ...systemUserForm, name: e.target.value })
                            }
                            required
                          />
                        </div>
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setSystemUserForm({ ...systemUserForm, email: e.target.value })
                            }
                            required
                          />
                        </div>
                        ...
                        {systemUserError && (
                          <p className="text-sm text-red-600">{systemUserError}</p>
                        )}
